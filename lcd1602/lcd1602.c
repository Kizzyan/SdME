...
static void WriteCommand(uint8_t ucCMD) {
  ...
  i2c_write_blocking(i2c, i2c_addr, &uc, 1, false);
  sleep_us(PULSE_PERIOD); // manually pulse the clock line
  uc |= 4; // enable pulse
  i2c_write_blocking(i2c, i2c_addr, &uc, 1, false);
...
int lcd1602Control(int bBacklight, int bCursor, int bBlink) {
  uint8_t ucCMD = 0xc; // display control
...
int lcd1602WriteString(char *text) {
    ...
    i2c_write_blocking(i2c, i2c_addr, ucTemp, 1, false);
    sleep_us(PULSE_PERIOD);
    ucTemp[0] |= 4; // pulse E
    i2c_write_blocking(i2c, i2c_addr, ucTemp, 1, false);
    sleep_us(PULSE_PERIOD);
...
//
int lcd1602Init(i2c_inst_t *i2c_instance, uint8_t iAddr) {
    i2c = i2c_instance;
